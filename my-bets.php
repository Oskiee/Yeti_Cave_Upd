...
// select items for bids
foreach ($bets as $key => $bet) {
    $sql = "SELECT * FROM items WHERE id = " . $bet['item_id'];
    $result = mysqli_query($conn, $sql);
    $lot = mysqli_fetch_assoc($result);
    $bets[$key]['title'] = $lot['title'];
    $bets[$key]['description'] = $lot['description'];
    $bets[$key]['starting_bid'] = $lot['starting_bid'];
    $bets[$key]['image_url'] = $lot['image_url'];
    $bets[$key]['created_at'] = $lot['created_at'];
    $bets[$key]['expires_at'] = $lot['expires_at'];
    $bets[$key]['min_bid'] = $lot['min_bid'];
    $bets[$key]['category_id'] = $lot['category_id'];
}
                ...
                <?php foreach ($bets as $bet): ?>
                    <tr class="rates__item">
                        ...
                        <td class="rates__category">
                            <?= $categories[$bet['category_id'] - 1]['title']; ?>
                        </td>
                        <td class="rates__timer">
                            <div class="timer <?php if (get_dt_range($bet['expires_at'] < 1)): ?>timer--finishing<?php endif; ?>">
                                <!-- Выводим часы и минуты -->
                                <?= get_dt_range($bet['expires_at'])[0] . 'h ' . get_dt_range($bet['expires_at'])[1] . 'min'; ?>
                            </div>
                        </td>
...
<footer class="main-footer">
    ...
    <div class="main-footer__bottom container">
        ...
        <?php endif; ?>        <div class="main-footer__developed-by">
