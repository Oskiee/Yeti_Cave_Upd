...
<div class="page-wrapper">
    ...
    <main>
        ...
        <div class="container">
            <section class="rates">
                <h2>Все лоты в категории <span>«<?= $categories[$cat_id - 1]['title']; ?>»</span></h2>
                <ul class="rates__list">
                    <?php foreach($lots as $lot): ?>
                    <li class="rates__item rates__item--history">
                        <div class="rates__info">
                            <div class="rates__img">
                                <img src="<?= $lot['image_url']; ?>" width="54" height="40" alt="<?= $lot['title']; ?>">
                            </div>
                            <h3 class="rates__title"><a href="lot.php?id=<?= $lot['id']; ?>"><?= $lot['title']; ?></a></h3>
                        </div>
                        <div class="rates__category">
                            <?= $categories[$cat_id - 1]['title']; ?>
                        </div>
                        <div class="rates__timer">
                            <dic class="timer <?php if (get_dt_range($lot['expires_at'])[0] < 1): ?>timer--finishing<?php endif; ?>">
                                <?= get_dt_range($lot['expires_at'])[0] . 'h ' . get_dt_range($lot['expires_at'])[1] . 'min'; ?>
                        </div>
                        <div class="rates__price">
                            <?= formatSum($lot['starting_bid']); ?>
                        </div>
                        <div class="rates__time">
                            <?=$lot['expires_at']?>
                        </div>
                    </li>
                    <?php endforeach; ?>
