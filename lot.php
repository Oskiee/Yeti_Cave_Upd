...
if (!$link) {
    ...
    print("Ошибка MySQL 1: " . $error);
    die();
}

// get user id from table by username if user is authorized
if ($is_auth) {
...
if (isset($_GET['id'])) {
    ...
    if ($result_items) {
        $lot = mysqli_fetch_assoc($result_items);
        if (!$lot) {
            http_response_code(404);
            $page_content = include_template('404.html', ['categories' => $categories]);
        }
    } else {
        http_response_code(404);
        $page_content = include_template('404.html', ['categories' => $categories]);
    ...
    $sql = "SELECT * FROM bids WHERE item_id = $id";
    $result = mysqli_query($link, $sql);
    if ($result) {
        $bids = mysqli_fetch_all($result, MYSQLI_ASSOC);
    } else {
        $error = mysqli_error($link);
        print("Ошибка MySQL 3: " . $error);
        die();
    }
    // add bid
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors) && $id !== null && $is_auth) {
            $sql = "INSERT INTO bids (item_id, bid_amount, user_id) VALUES (?, ?, ?)";
            $stmt = mysqli_prepare($link, $sql);
            mysqli_stmt_bind_param($stmt, 'iii', $id, $bid, $user_id);
            mysqli_stmt_execute($stmt);
            header("Location: /localhost/lot.php?id=" . $id);
        }
    }
} else {
    http_response_code(404);
            ...
            <section class="lot-item container">
                ...
                <div class="lot-item__content">
                    <div class="lot-item__left">
                        ...
                        <p class="lot-item__category">Категория: <span><?= $categories[$lot['category_id'] - 1]['title']; ?></span></p>
                        <p class="lot-item__description"><?= $lot['description']; ?></p>
                    </div>
                    <div class="lot-item__right">
                        <div class="lot-item__state">
                            <div class="lot-item__timer timer <?= get_dt_range($lot['expires_at'])[0] < 1 ? 'timer--finishing' : ''; ?>">
                                <?= get_dt_range($lot['expires_at'])[0] . 'h ' . get_dt_range($lot['expires_at'])[1] . 'min'; ?>
                            </div>
                            <div class="lot-item__cost-state">
                                <div class="lot-item__rate">
                                    <span class="lot-item__amount">Текущая цена</span>
                                    <span class="lot-item__cost"><?= $lot['starting_bid']; ?></span>
                                </div>
                            ...
                            <form class="lot-item__form" action="lot.php?id=<?= $id; ?>" method="post">
                                ...
                                <input id="cost" type="number" name="cost" placeholder="<?= $lot['min_bid']; ?>">
                                <?php if ($is_auth): ?>
                                    <button type="submit" class="button">Сделать ставку</button>
                                <?php else: ?>
                                    <button type="submit" class="button" disabled>Сделать ставку</button>
                                    <span>Чтобы сделать ставку, <a href="login.php">войдите на сайт</a></span>
                                <?php endif; ?>
                            ...
                            <table class="history__list">
                                <?php foreach($bids as $bid): ?>
                                    <tr class="history__item">
                                        <td class="history__name"><?= $users[$bid['user_id'] - 1]['username']; ?></td>
                                        <td class="history__price"><?= $bid['bid_amount']; ?> р</td>
                                        <td class="history__time"><?= $bid['bid_time']; ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </table>
                        </div>
