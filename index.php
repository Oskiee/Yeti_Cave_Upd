...
$sql = "SELECT * FROM `items` ORDER BY `expires_at` ASC";
$result = mysqli_query($conn, $sql);
...
<main class="container">
        ...
        <ul class="promo__list">
            ...
            <?php foreach($categories as $category): ?>
                <li class="promo__item promo__item--<?= $category['code']; ?>">
                    <a class="promo__link" href="all-lots.php?id=<?= $category['id']; ?>"><?= $category['title']; ?></a>
                </li>
            <?php endforeach; ?>
        </ul>
    </section>
    ...
    <?php foreach($lots as $lot): ?>
        <li class="lots__item lot">
            ...
            <div class="lot__info">
                <span class="lot__category"><?= $categories[$lot['category_id'] - 1]['title']; ?></span>
                <h3 class="lot__title"><a class="text-link" href="lot.php?id=<?= $lot['id']; ?>"><?= $lot['title']; ?></a></h3>
                <div class="lot__state">
                    <div class="lot__rate">
                        <span class="lot__amount"><?= formatSum($lot['starting_bid']); ?><b class="rub">₽</b></span>
                    </div>
                    <div class="lot__timer timer">

                     <?php
                        $endDate = $lot['expires_at'];  // Получаем время окончания из массива
                        list($hours, $minutes) = get_dt_range($endDate);  // Получаем массив с часами и минутами
                     ?>
                     <!-- return categories sorted by time left -->
                    <span><?= $hours ?> hours and <?= $minutes ?> minutes left</span>
