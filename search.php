<?php
//Полнотекстовый поиск — это возможность MySQL, которая позволяет искать записи в таблице
//по нестрогому совпадению текста. Используйте его, когда необходимо добавить на сайт поиск
//по отдельным сущностям с текстовым наполнением.
//Задание
//Оживите форму поиска в шапке сайта. Эта форма будет искать лоты по их названию или
//описанию. Найденные лоты будут показаны на отдельной странице поиска.
//Список действий:
//1. В schema.sql добавьте SQL-инструкцию на создание полнотекстового индекса для полей
//«название» и «описание» в таблице лотов. Этот же запрос выполните в своей MySQL.
//2. Форма поиска должна вести на файл search.php и работать по методу GET.
//3. Сделайте новый шаблон для страницы поиска (вёрстка в pages/search.html ).
//4. Добавьте новый сценарий search.php , который будет искать лоты и показывать результат
//в шаблоне.
//5. Если по запросу нет результатов, то на их месте должна быть надпись «Ничего не найдено
//по вашему запросу».
//Проверка
//1. Выполните запрос на создание полнотекстового индекса.
//2. Введите название одного из существующих лотов в форму поиска и нажмите Enter.
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($search) {
        $sql = "SELECT l.id, l.title, l.starting_bid, l.image_url, c.title AS category, l.expires_at FROM items l
...
function formatSum($sum) {
    $ceil_val = ceil($sum); // Округлить число до целого используя функцию ceil().

    if ($ceil_val < 1000) return $ceil_val . "₽";  // Если переданное число меньше 1000, то оставить его как есть.

    $last_three = substr($sum,-3);  // отделить три последних цифры от остальной части суммы.

    return substr($sum,0,-3) . " " . $last_three . "₽";   // добавить пробелом и знак рубля — ₽.
}

// Функция расчета времени до окончания лота
function get_dt_range($endDate) {
    $ts = strtotime($endDate) - time();  // Вычисляем разницу между текущим временем и временем окончания лота
    $hours = floor($ts / 3600);  // Округляем в меньшую сторону и получаем часы
    $minutes = floor(($ts % 3600) / 60);  // Округляем в меньшую сторону и получаем минуты
    return [$hours, $minutes];  // Возвращаем результат в виде массива
}
?>
...
<div class="page-wrapper">
    ...
    <header class="main-header">
        <div class="main-header__container container">
            ...
            <?php else: ?>
                <a class="main-header__add-lot button" href="login.php">Войти</a>
            <?php endif; ?>
            <nav class="user-menu">

                <?php
                if ($is_auth) { // Если пользователь авторизован
                    echo '<div class="user-menu__logged"><p>'. $user_name .'</p><a class="user-menu__bets" href="my-bets.php">Мои ставки</a><a class="user-menu__logout" href="logout.php">Выход</a></div>';
                } else { // Если пользователь не авторизован
                    echo '<ul class="user-menu__list"><li class="user-menu__item"><a href="sign-up.php">Регистрация</a></li><li class="user-menu__item"><a href="login.php">Вход</a></li></ul>';
                }
                ?>
        ...
        <nav class="nav">
            <ul class="nav__list container">
                <?php foreach ($categories as $category): ?>
                    <li class="nav__item">
                        <a href="all-lots.php?id=<?= $category['id']; ?>"><?= $category['title']; ?></a>
                    </li>
                <?php endforeach; ?>
            </ul>
        </nav>
            ...
            <li class="lots__item lot">
                ...
                <div class="lot__info">
                    ...
                    <div class="lot__state">
                        <div class="lot__rate">
                            <span class="lot__amount"><?= formatSum($lot['starting_bid']); ?></span>
                        </div>
                        <div class="lot__timer timer">
                            <?= get_dt_range($lot['expires_at'])[0] . 'h ' . get_dt_range($lot['expires_at'])[1] . 'min'; ?>
                        </div>
                    </div>
                </div>
            </li>
